...
class Company {
  ...
  /** Find all companies.
   * if passed query params will filter results. valid params = name, maxEmployees, minEmployees
   * Returns [{ handle, name, description, numEmployees, logoUrl }, ...]
   * */

  static async findAll(data) {
    let minEmployees, maxEmployees
    if (data && "minEmployees" in data && "maxEmployees" in data){
        ({minEmployees, maxEmployees} = data)
    }
    if (maxEmployees && minEmployees && maxEmployees < minEmployees) {
      throw new BadRequestError("Max should be greater than min Employees.");
    }
